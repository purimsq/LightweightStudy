...
function CreateAssignmentDialog({ isOpen, onOpenChange, units }: { 
  ...
  const createMutation = useMutation({
    mutationFn: async (data: { title: string; description: string; dueDate?: string; unitId?: number }) => {
...
function UploadAssignmentDialog({ isOpen, onOpenChange, units }: { 
  isOpen: boolean; 
  onOpenChange: (open: boolean) => void;
  units: Unit[];
}) {
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Assignment uploaded successfully!" });
    },
    onError: (error) => {
      toast({ 
        title: "Error uploading assignment", 
        description: error.message,
        variant: "destructive" 
      });
    }
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <Label htmlFor="upload-unit">Unit (optional)</Label>
            <Select value={unitId} onValueChange={setUnitId}>
              <SelectTrigger>
                <SelectValue placeholder="Select a unit..." />
              </SelectTrigger>
